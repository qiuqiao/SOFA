...
def get_vlabeler_edit_ratio(
    pred: tg.PointTier, target: tg.PointTier, move_tolerance=20
):
    ...
    def dfs(i, j):
        ...
        # 如果音素不一致就要修改，否则不需要
        move = dfs(i - 1, j - 1)
        if abs(pred[i - 1].time - target[j - 1].time) > move_tolerance:
            move += 1
    ...
    edit_num = dfs(len(pred), len(target))
    return edit_num / max_edit_num, edit_num
