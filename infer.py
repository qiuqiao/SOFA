...
class DataItem(metaclass=ABCMeta):
    ...
    def align(self, aligner, *args, **kwargs):
        # aligner:Aligner
        aligned_res, align_information = aligner(
            self.input_audio, self.input_seq, *args, **kwargs
        )
...
class AddAspiration(TextGridProcessor):
        ...
        for i in tier1:
            for j in tier2:
                if i.xmax > j.xmin and i.xmin < j.xmax:
                    intersection_tier.append(
                        tg.Interval(
                            text=text,
                            xmin=max(i.xmin, j.xmin),
                            xmax=min(i.xmax, j.xmax),
                        )
                    )
    ...
    def __call__(self, data_item: DataItem) -> Any:
        ...
        tier_of_empty = tg.Tier(
            [i for i in data_item.aligned_tg["phones"] if i.text == ""]
        )
        intersection_tier = self.tier_intersection(tier_of_ap, tier_of_empty)
        intersection_tier = tg.Tier(
            [i for i in intersection_tier if i.xmax - i.xmin > args.min_ap_interval_dur]
        )
        data_item.aligned_tg["words"] = [
            i for i in data_item.aligned_tg["words"] if i.text != ""
        ]
        data_item.aligned_tg["phones"] = [
            i for i in data_item.aligned_tg["phones"] if i.text != ""
        ]
        data_item.aligned_tg["words"] = self.add_intervals(
            data_item.aligned_tg["words"], intersection_tier
        )
        data_item.aligned_tg["phones"] = self.add_intervals(
            data_item.aligned_tg["phones"], intersection_tier
        )
        data_item.aligned_tg["words"] = fill_in_empty_of_tier(
            data_item.aligned_tg["words"],
            length=data_item.input_audio.shape[-1] / config.sample_rate,
            text="SP",
        )
        data_item.aligned_tg["phones"] = fill_in_empty_of_tier(
            data_item.aligned_tg["phones"],
            length=data_item.input_audio.shape[-1] / config.sample_rate,
            text="SP",
        )
...
class WordDictionary(Dictionary):
    ...
    def load(self, file_path: str):
        ...
        self.dict = {
            i.split("\t")[0].strip(): i.split("\t")[1].strip().split(" ")
            for i in dictionary
        }
...
class Aligner:
    ...
    def decode_alignment(
        ...
        for i in range(1, dp.shape[-1]):
            ...
            backtrack_j[:, i] = np.arange(len(prob1)) - np.argmax(
                np.stack([prob1, prob2, prob3], axis=0), axis=0
            )
    ...
    def __call__(
        ...
        ph_seq_num_pred, ph_time_pred_int, frame_confidence = self.decode_alignment(
            ph_seq_num, prob_log, is_edge_prob_log, not_edge_prob_log
        )
        ...
        phones_tier = intervals_to_tier(
            ph_interval_pred, ph_seq_pred, ignore_text_list=[vocab[0]]
        )
        ...
        if return_plot:
            plot1 = utils.plot_spectrogram_and_phonemes(
                melspec[0].cpu(),
                target_gt=frame_confidence * config.n_mels,
                ph_seq=ph_seq_pred,
                ph_dur=ph_dur_pred,
            )
            plot2 = utils.plot_spectrogram_and_phonemes(
                seg_prob, target_gt=edge_pred * vocab["<vocab_size>"]
            )  # target_pred=frame_target,
...
class DataItemOfWordLab(DataItem):
    ...
    def add_words_tier(self, phones_tg, word_seq, ph_num):
        ...
        for i in range(len(ph_location) - 1):
            if i > 0 and words_tier[-1].xmax != phones_tier[ph_location[i]].xmin:
                words_tier.append(
                    tg.Interval(
                        "", words_tier[-1].xmax, phones_tier[ph_location[i]].xmin
                    )
                )
            words_tier.append(
                tg.Interval(
                    word_seq[i],
                    phones_tier[ph_location[i]].xmin,
                    phones_tier[ph_location[i + 1] - 1].xmax,
                )
            )
        # print(phones_tg)
    ...
    def postprocess_align(self):
        self.aligned_tg = self.add_words_tier(
            self.aligned_tg, self.word_seq, self.ph_num
        )
...
def get_ap_interval(audio):
    ...
    min_frame_length = int(
        (args.min_ap_interval_dur) / (config.hop_length / config.sample_rate) + 0.5
    )
    is_ap_diff = torch.diff(
        torch.cat(
            (
...
def interval_intersection(intervals_a=[[1, 3], [6, 8]], intervals_b=[[2, 6]]):
    ...
    while idx_a < len(intervals_a) and idx_b < len(intervals_b):
        if intervals_a[idx_a][0] > intervals_b[idx_b][1]:
            ...
            intersection.append(
                [
                    max(intervals_a[idx_a][0], intervals_b[idx_b][0]),
                    min(intervals_a[idx_a][1], intervals_b[idx_b][1]),
                ]
            )
...
def detect_AP(audio_path, ph_seq, ph_interval):
    ...
    output_interval, interval_idx = interval_intersection(
        empty_interval, get_ap_interval(audio_path)
    )
    if len(interval_idx) < 1:
        return []

    output_interval = np.array(
        [i for i in output_interval if i[1] - i[0] > args.min_ap_interval_dur]
    )
    ...
    for i in range(len(text)):
        if text[i] in ignore_text_list:
            ...
            intervals_list.append(
                tg.Interval(text[i], intervals[i, 0], intervals[i, 1])
            )
...
def parse_args():
    ...
    parser.add_argument(
        ...
        type=str,
        default="segments",
        help="segments path. It shoould contain 2 types of file: .wav and .lab",
    )
    parser.add_argument(
        "-d",
        "--dictionary_path",
        type=str,
        default=os.path.join("dictionary", "opencpop-extension.txt"),
        help="dictionary path. It should be a txt file.",
    )
    parser.add_argument(
        "-p", "--phoneme_mode", action="store_true", help="enable phoneme mode"
    )

    parser.add_argument("--br_db", type=float, default=35, help="breath db threshold")
    parser.add_argument(
        "--br_centroid", type=float, default=2000, help="breath centroid threshold"
    )
    parser.add_argument(
        "--chromagram_entropy_thresh",
        type=float,
        default=2.5,
        help="chromagram entropy threshold",
    )
    parser.add_argument(
        "--min_ap_interval_dur",
        type=float,
        default=0.1,
        help="min ap interval duration",
    )
    parser.add_argument(
        "--empty_count_penalty_weight",
        type=float,
        default=0.5,
        help="empty count penalty weight",
    )
    parser.add_argument(
        "--empty_length_penalty_weight",
        type=float,
        default=0.5,
        help="empty length penalty weight",
    )
    ...
    for path, subdirs, files in os.walk(args.segments_path):
        if len(subdirs) == 0:
            ...
            file_names = list(
                ...
                & set(
                    map(lambda x: x[:-4], filter(lambda x: x.endswith(".wav"), files))
                )
            )
