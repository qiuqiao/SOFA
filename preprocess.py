        ...
        if os.path.exists(
            ...
            trans["path"] = trans.apply(
                lambda name: os.path.join(
                    "data", folder, dataset_name, "raw", "wavs", name["name"] + ".wav"
                ),
                axis=1,
            )
...
def full_label_binarize(data_list, name="train"):
    ...
    for index in trange(len(data_list)):
        ...
        if T > config.melspec_maxlength:
            warnings.warn(
                f'Melspec of {data_list.iloc[index]["path"][:-4]} has a length of {T}, which is longer than {config.melspec_maxlength}. Ignored.'
            )
            continue
        ...
        ph_dur = torch.tensor(
            [float(i) for i in data_list.iloc[index]["ph_dur"].split(" ") if i != ""]
        )
