...
def init_config(config_input):
    ...
    spec_transform = torchaudio.transforms.Spectrogram(
        n_fft=config.chromagram_n_fft,
        win_length=None,
        hop_length=config.hop_length,
        power=2.0,
    ).to(config.device)
    chroma_fbank = get_chroma_fbank(config.sample_rate, config.chromagram_n_fft, 24).to(
        config.device
    )
    spectral_centroid_transform = torchaudio.transforms.SpectralCentroid(
        config.sample_rate,
        config.chromagram_n_fft,
        win_length=None,
        hop_length=config.hop_length,
    ).to(config.device)
...
def get_chroma_spec(audio):
    ...
    chroma_spec = torch.nn.functional.softmax(
        1 + torch.log((spec.T @ chroma_fbank).T), dim=0
    )
    return chroma_spec
...
def get_loudness_SPL(audio):
    weight = (
        torch.hann_window(config.hop_length * 2, periodic=False)
        .unsqueeze(0)
        .unsqueeze(0)
        .to(config.device)
    )
    ...
    output = torch.nn.functional.conv1d(
        torch.pow(audio, 2), weight, stride=config.hop_length, padding=config.hop_length
    )
...
def extract_normed_mel(waveform):
    ...
    melspec = F.amplitude_to_DB(
        melspec, multiplier=10.0, amin=1e-10, db_multiplier=1.0, top_db=90.0
    )
...
def read_ndarray_from_bin(file, idx_data):
    ...
    return np.frombuffer(file.read(idx_data["len"]), dtype=idx_data["dtype"]).reshape(
        idx_data["shape"]
    )
...
def load_resampled_audio(audio_path):
    ...
    if sample_rate != config.sample_rate:
        audio = torchaudio.transforms.Resample(sample_rate, config.sample_rate).to(
            config.device
        )(audio)
    return audio
