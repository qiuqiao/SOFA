...
if __name__ == '__main__':
    ...
    full_train_dataloader = BinaryDataLoader(
        dataset_file_path='data/full_label/train',
        data_names=['input_feature','seg_target','edge_target'],
        batch_size=config.batch_size_sup*2,
        instances_buffer_size=3000,
        num_process=9)
    ...
    for i in range(config.max_steps):
        ...
        if usp_scheduler()>0:
            ...
            # loss+=0.1*usp_scheduler()*consistence_loss
        ...
        if i%config.val_interval==0:
            ...
            #         melspec,target,edge_target=melspec.to(config.device),target.to(config.device),edge_target.to(config.device)
            #         h,seg,ctc,edge=model(melspec)
        ...
        if config.test_interval>0 and i%config.test_interval==0:
            pass
            # print('testing...')
            # model.eval()
            # id=1
