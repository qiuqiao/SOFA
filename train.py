...
def parse_args():
    ...
    parser.add_argument(
        ...
        type=str,
        required=True,
        help="model folder name in /ckpt.",
    )
...
if __name__ == "__main__":
    ...
    full_train_dataloader = DataLoader(
        ...
        batch_size=config.train.batch_size_sup,
        shuffle=True,
    ...
    weak_train_dataloader = DataLoader(
        dataset=weak_train_dataset,
        batch_size=config.train.batch_size_wsp,
        shuffle=True,
        collate_fn=weak_label_collate_fn,
    )
    weak_train_dataiter = iter(weak_train_dataloader)

    weak_valid_dataset = WeakLabelDataset(name="valid")
    weak_valid_dataloader = DataLoader(
        dataset=weak_valid_dataset,
        batch_size=config.train.batch_size_wsp,
        shuffle=False,
        collate_fn=weak_label_collate_fn,
        drop_last=True,
    ...
    # loss function
    seg_GHM_loss_fn = utils.GHMLoss(
        vocab["<vocab_size>"],
        num_prob_bins=10,
        alpha=0.999,
        label_smoothing=config.label_smoothing,
    )
    edge_GHM_loss_fn = utils.GHMLoss(
        2, num_prob_bins=5, alpha=0.999999, label_smoothing=0.0, enable_prob_input=True
    )
    ...
    scheduler = OneCycleLR(
        optimizer, max_lr=config.train.learning_rate, total_steps=config.train.max_steps
    )
    wsp_scheduler = GaussianRampUpScheduler(
        config.train.max_steps, 0, config.train.max_steps
    )
    ...
    for step in range(config.train.max_steps):
        ...
        edge_diff_loss = MSE_loss_fn(
            torch.diff(is_edge_prob, dim=-1), torch.diff(edge_target[:, 0, :], dim=-1)
        )
        ...
        # log
        writer.add_scalar(
            "Accuracy/train/fsp/accuracy",
            (seg.argmax(dim=1) == target).float().mean().item(),
            step,
        )
        ...
        ctc_loss = CTC_loss_fn(
            ctc_log_softmax,
            ctc_target,
            torch.tensor(ctc_log_softmax.shape[0]).repeat(ctc_log_softmax.shape[1]),
            ctc_target_lengths,
        )
        wsp_loss = ctc_loss
        ...
        if step % config.train.val_interval == 0:
            ...
            with torch.no_grad():
                for melspec, target, edge_target in full_valid_dataloader:
                    melspec, target, edge_target = (
                        melspec.to(config.device),
                        target.to(config.device).squeeze(1),
                        edge_target.to(config.device).squeeze(1).long(),
                    )
                    h, seg, ctc, edge = model(melspec)
            ...
            confusion_matrix = utils.confusion_matrix(
                vocab["<vocab_size>"], y_pred, y_true
            )
                ...
                for (
                    ...
                    ctc_losses.append(
                        CTC_loss_fn(
                            ctc,
                            ctc_target,
                            torch.tensor(ctc.shape[0]).repeat(ctc.shape[1]),
                            ctc_target_lengths,
                        )
                        .cpu()
                        .item()
            ...
            for path, subdirs, files in os.walk(os.path.join("data", "test")):
                for file in files:
                    if file == "transcriptions.csv":
                        ...
                        trans["path"] = trans.apply(
                            lambda x: os.path.join(path, "wavs", x["name"] + ".wav"),
                            axis=1,
                        )
                        ...
                        for idx in trange(len(trans)):
                            ...
                            writer.add_text(
                                f"{id}/ctc_ph_seq", " ".join(others["ctc_ph_seq"]), step
                            )
                            id += 1

                            ph_confidence_total.append(others["confidence"])
                        writer.add_scalar(
                            "Accuracy/test/confidence",
                            np.mean(ph_confidence_total),
                            step,
                        )
            model.train()
        ...
        if (
            ...
            torch.save(
                model.state_dict(),
                os.path.join("ckpt", config.model_name, f"{step}.pth"),
            )
            print(
                f"saved model at {step} steps, path: ckpt/{config.model_name}/{step}.pth"
            )
